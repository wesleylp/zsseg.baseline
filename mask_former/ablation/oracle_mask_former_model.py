...
def compute_iou(mask_m, mask_n):
    m = mask_m.shape[0]
    n = mask_n.shape[0]
    mask_m = mask_m.reshape(m, -1)
    mask_n = mask_n.reshape(n, -1)
    intersection = torch.sum((mask_m[:, None, :] + mask_n[None, ...]) > 1, dim=-1)
    union = torch.sum((mask_m[:, None, :] + mask_n[None, ...]) > 0, dim=-1)
...
class OracleMaskFormer(nn.Module):
    """
    ...
    def semantic_inference(self, mask_cls, mask_pred, gt_instances):
